          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return  QuizSetCard(
                ...
                onUpdate: (status) {
                  vm.changeQuizSetStatus(context, quizSet, folderId, status);
                },
              );
            },
          ),
...
// ─── Quiz Set Card ────────────────────────────────────────────────────────────
/// A quiz set counts as published only when the server status says so;
/// anything else (null, `draft`, …) is treated as a draft.
bool _statusIsPublished(String? status) =>
...
class QuizSetCard extends StatelessWidget {
  ...
  /// Called with the new status (`published` or `draft`).
        ...
        child: GestureDetector(
          ...
          child: ListTile(
            ...
            trailing: _QuizSetCardMenu(
              ...
              onDelete: onDelete,
              onUpdate: onUpdate,
              isPublished: _statusIsPublished(quizSet.status),
            ),
          ),
        ),
      ),
...
class _PublishedBadge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final published = _statusIsPublished(status);
    return Container(
